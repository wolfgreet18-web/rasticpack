/**
 * ⚠️ وضعیت: این فایل در مسیر فعال Customer استفاده نمی‌شود.
 * html8.html مستقیماً از '../src/db/CustomerRepo.js' import می‌کند (نه از
 * www/js/db/). این کپی احتمالاً باقی‌مانده‌ی یک ساختار پوشه‌ی قدیمی‌تر
 * (www/ به‌عنوان ریشه‌ی Capacitor) است. برای جلوگیری از drift/گمراهی
 * آینده، محتوایش با src/db/CustomerRepo.js هماهنگ نگه داشته شده — ولی
 * تا وقتی importکننده‌ای نداشته باشد، صرفاً یک نسخه‌ی بی‌اثر (dead code)
 * است. قبل از حذف کامل (که این بررسی عمداً انجامش نداد)، مطمئن شوید
 * بیلد/باندل اندروید (esbuild سراغ src/sqlite-entry.js می‌رود، نه اینجا)
 * واقعاً به این مسیر وابسته نیست.
 */
/**
