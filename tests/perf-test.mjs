// تست کارایی — قبلاً در نسخه‌ی ۴.۲ سند فقط به‌صورت یک اسکریپت یک‌بارمصرف اجرا و
// نتیجه‌اش (۱۴ms) در roadmap ذکر شده بود، بدون اینکه خودِ اسکریپت جایی ذخیره شود
// (یعنی غیرقابل‌بازتولید بود). این نسخه همان تست را به‌صورت یک فایل دائمی در
// tests/ ذخیره می‌کند تا هر زمان لازم بود، دوباره قابل اجرا و تأیید باشد.
//
// اجرا: node --experimental-sqlite perf-test.mjs
